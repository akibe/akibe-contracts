...
describe('Owner Mint', function () {
  ...
  it('Minter以外はMintできない', async function () {
    await expect(
      obj.contract.connect(obj.guest).mint(obj.addr[1].address, 1)
  ...
  it('複数トークンを一度にMintできる', async function () {
    await expect(obj.contract.connect(obj.owner).mint(obj.addr[0].address, 10))
      .to.emit(obj.contract, 'Transfer')
      .withArgs(ethers.ZeroAddress, obj.addr[0].address, 11)
    expect(await obj.contract.totalSupply()).to.equal(11)
    expect(await obj.contract.balanceOf(obj.addr[0].address)).to.equal(11)
  })
})
