...
describe('TokenURI', function () {
  ...
  before(async function () {
    ...
    await obj.contract.connect(obj.owner).mint(obj.guest.address, 2)
  })
  ...
  it('所有トークンを取得できる', async function () {
    const mint1 = obj.contract.connect(obj.owner).mint(obj.addr[1].address, 3)
    await expect(mint1)
      .to.emit(obj.contract, 'Transfer')
      .withArgs(ethers.ZeroAddress, obj.addr[1].address, 3)

    const mint2 = obj.contract.connect(obj.owner).mint(obj.addr[1].address, 4)
    await expect(mint2)
      .to.emit(obj.contract, 'Transfer')
  ...
  it('無効なアドレス指定はエラーになる', async function () {
    ...
    ).to.be.revertedWithCustomError(obj.contract, 'ERC721InvalidOwner')
  })
